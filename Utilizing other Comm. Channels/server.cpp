...
void process_file_request(RequestChannel* rc, char* request){
	...
	//cout << "Server received request for file " << filename << " with offset " << f.offset << " and length " << f.length << endl;

	if (f.offset == 0 && f.length == 0){ // means that the client is asking for file size
//cout << "server getting size of file " << filename << endl;
	    __int64_t fs = get_file_size(filename);
//cout << "SERVER WRITING FILELEN...";
...
//cout << "done." << endl;
	...
	if(f.length > buffercapacity){
		cerr << "F.LENGTH = " << f.length << endl << "F.OFFSET = " << f.offset << endl;
...
void process_data_request(RequestChannel* rc, char* request){
	...
	//cout << "SERVER: PROCCESSING REQUEST FOR POINT <p, t, e> = " << d->person << ',' << d->seconds << ',' << d->ecgno << endl;
...
int main(int argc, char *argv[]){
	...
	while ((opt = getopt(argc, argv, "m:i:")) != -1) {
		switch (opt) {
			case 'm':
				...
				//cout << "Argument -m passed... setting max buffer to " << optarg << endl;
				break;
			case 'i':
				ipcMethod = optarg;
				break;
			default:
				cout << "Unknown Arguement -" << opt << " passed to child server." << endl;
				exit(EXIT_FAILURE); 
		}
	}
    ...
    //cout << "Server joining control channel." << endl;
	// Set up control channel for all communication
