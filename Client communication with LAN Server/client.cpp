...
void event_polling_function(BoundedBuffer* reqBuf, TCPRequestChannel* chans[], HistogramCollection* hc, int mb, int w){
    char buf[mb];
    char recvBuf[mb];
...
//                cout << "FOUND QUIT MSG BEFORE W REQUESTS SENT\n";
            ...
            int chanrfd = chans[i]->getfd();
            fcntl(chanrfd, F_SETFL, O_NONBLOCK);
            fcntl(chans[i]->getfd(), F_SETFL, O_NONBLOCK);
...
void file_thread_function(string* fname, BoundedBuffer* reqBuf, TCPRequestChannel* chan, int mb) {
    if(fname->empty())
        return;
...
int main(int argc, char *argv[]) {
    ...
    auto** workerChans = new TCPRequestChannel*[w];

    for (int i = 0; i < w; i++)
        workerChans[i] = new TCPRequestChannel(host, port);
    ...
    thread filethread(file_thread_function, &f, &request_buffer, workerChans[0], m);
    ...
    int Tsecs = (end.tv_sec * 1e6 + end.tv_usec - Tstart.tv_sec * 1e6 - Tstart.tv_usec) / (int) 1e6;
    int Tusecs = (int) (end.tv_sec * 1e6 + end.tv_usec - Tstart.tv_sec * 1e6 - Tstart.tv_usec) % ((int) 1e6);
    int Dsecs = Tsecs-secs;
    int Dusecs = Tusecs-usecs;
    cout << "Operation Took " << secs << " seconds and " << usecs << " micro seconds" << endl;
    cout << "Total time was " << Tsecs << " seconds and " << Tusecs << " micro seconds" << endl;
    cout << "Time difference was " << Dsecs << " seconds and " << Dusecs << " micro seconds" << endl;
}